"""
Workflow del Agente GreenTravelBackend
=======================================

Este módulo define el flujo del Agente GreenTravelBackend utilizando LangGraph.
El agente puede interactuar con los servicios de Liquidaciones y Proveedores
de GreenTravelBackend mediante herramientas MCP.

HERRAMIENTAS DISPONIBLES:
- Liquidaciones: list_liquidaciones, get_liquidacion, create_liquidacion, 
  update_liquidacion, delete_liquidacion, get_liquidacion_stats
- Proveedores: list_provedores, get_provedor, create_provedor, 
  update_provedor, delete_provedor, get_provedor_stats

El agente puede realizar operaciones CRUD completas y consultar estadísticas
sobre liquidaciones y proveedores.
"""
...
class AgentState(TypedDict):
    """Estado del agente GreenTravelBackend."""
...
async def decide_node(state: AgentState, model, tools_by_name):
    """
    Nodo de decisión principal (ReAct). El LLM decide qué herramienta usar.
    """
    ...
    # Agregar mensaje del sistema con capacidades
...
async def tools_node(state: AgentState, tools_by_name):
    ...
    for call in tool_calls:
        ...
        if tool is None:
            result = f"Error: herramienta '{tool_name}' no existe. Herramientas disponibles: {', '.join(tools_by_name.keys())}"
        else:
            try:
                logger.info(f"[TOOLS] Ejecutando {tool_name} con parámetros: {tool_input}")
                result = await tool.ainvoke(tool_input)
                logger.info(f"[TOOLS] {tool_name} ejecutada exitosamente")
            except Exception as e:
                result = f"Error ejecutando herramienta {tool_name}: {str(e)}"
                logger.error(f"[TOOLS] {result}")
...
def build_greentravel_agent(model, tools_by_name):
    """
    Construye el grafo del agente GreenTravelBackend.
...
def visualize_graph(graph_instance=None):
    ...
    try:
        ...
        if graph_instance is None:
            ...
            # Crear herramientas mock para visualización
            mock_tools = {}
            tool_names = [
                # Liquidaciones
            ...
            for tool_name in tool_names:
                ...
                async def mock_tool_func(**kwargs):
                    ...
                    return f"Mock {tool_name} response"
                mock_tool_func.__name__ = tool_name
                mock_tools[tool_name] = mock_tool_func
            
            graph_instance = build_greentravel_agent(llm.bind_tools(list(mock_tools.values())), mock_tools)
        ...
        output_path = images_dir / "greentravel_agent_graph.png"
